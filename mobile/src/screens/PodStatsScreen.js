...
export default function PodStatsScreen({ navigation, route }) {
  ...
  const podName = route?.params?.podName || 'Circle'
  ...
  const getInsights = () => {
    if (!stats) return []
    const lines = []
    if (stats.avg_health >= 80) lines.push('Your circle is crushing it! Keep the momentum.')
    else if (stats.avg_health >= 60) lines.push('Solid consistency. Push for Gold tier.')
    else lines.push('The circle needs a reset. Rally your members.')
  ...
  const renderChart = () => {
    ...
    return (
      <View style={s.chartWrap}>
        ...
        <Text style={[s.chartLabel, { color: colors.textSec }]}>Circle Health Trend</Text>
      </View>
    )
  }
          ...
          {podBest && (
            <View style={s.podBestRow}>
              ...
              <Text style={[s.podBestText, { color: colors.textSec }]}>
                Circle best: {podBest.longest_streak} days by {podBest.name}
              </Text>
            </View>
          )}
        </View>
