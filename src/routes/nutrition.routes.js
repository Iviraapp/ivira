...
export default async function nutritionRoutes(fastify) {
  ...
  // Log a meal
  fastify.post('/gyms/:gymId/members/:memberId/nutrition/log', authHooks, async (request) => {
  ...
  // Get daily log
  fastify.get('/gyms/:gymId/members/:memberId/nutrition/daily', authHooks, async (request) => {
    const { gymId, memberId } = request.params;
  ...
  // Get weekly trend
  fastify.get('/gyms/:gymId/members/:memberId/nutrition/weekly', authHooks, async (request) => {
    const { gymId, memberId } = request.params;
    return nutritionService.getWeeklyTrend(memberId, gymId);
  });

  // Get nutrition goal
  fastify.get('/gyms/:gymId/members/:memberId/nutrition/goal', authHooks, async (request) => {
    const { gymId, memberId } = request.params;
    return nutritionService.getNutritionGoal(memberId, gymId);
  });

  // Update nutrition goal
  fastify.patch('/gyms/:gymId/members/:memberId/nutrition/goal', authHooks, async (request) => {
    const { gymId, memberId } = request.params;
